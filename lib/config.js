...
var ENV_MAPPING = {
  ...
  proxy_ssl                   : "NEW_RELIC_PROXY_SSL",
  proxy_ca                    : "NEW_RELIC_PROXY_CA",
  proxy_cert                  : "NEW_RELIC_PROXY_CERT",
  proxy_key                   : "NEW_RELIC_PROXY_KEY",
 ...
 */
Config.prototype.setLogger = function (bootstrapped) {
  logger = bootstrapped;
};
    ...
    if (typeof node === 'string') {
      ...
      if (setting) {
        if (LIST_VARS.indexOf(node) > -1) {
          data[value] = setting.split(',').map(function (k) { return k.trim(); });
        }
